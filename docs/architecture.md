...
|---|-------------|----------|-------------|
| 1 | Appearance preferences are client-only — persisted in `localStorage` under `vite-ui-*` keys (`display-mode`, `header-style`, `font`, `bg-url`, `custom-colors`, `frost`, `animations`, `clean-view`, `border-opacity`, `border-width`, `border-line-style`, `sidebar-collapsed`). Runtime theming applied by setting inline CSS vars on `document.documentElement` and toggling classes (`frost`, `animations`, `clean-view`). Tropical preset defaults are seeded on first visit by an inline script in `index.html`. | `src/frontend/src/components/settings/appearance-store.ts`, `src/frontend/index.html` | Accepted; no backend persistence planned |
